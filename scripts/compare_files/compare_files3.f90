...
PROGRAM compare_files3
    ...
    DO n = 0, ngrids-1
        ...
        ALLOCATE(q1(mx,my,mz,meqn))
        ALLOCATE(q2(mx,my,mz,meqn))
        ALLOCATE(qc(mx,my,mz,meqn))
        ...
        do k = 1,mz
            DO j = 1,my
                DO i = 1,mx
                    READ(10,*) (q1(i,j,k,m),m = 1,meqn)
                    READ(20,*) (q2(i,j,k,m),m = 1,meqn)
                    DO m = 1,meqn
                        qc(i,j,k,m) = q1(i,j,k,m)-q2(i,j,k,m)
                        IF (ABS(qc(i,j,k,m)) .GT. global_max(m)) THEN
                            global_max(m) = ABS(qc(i,j,k,m))  !! Largest difference on all grids
                            grid_max(m) = ngrid        !! max occurs on this grid
                        END IF
                        max_grids(ngrid,m) = MAX(max_grids(ngrid,m),ABS(qc(i,j,k,m)))
                    END DO
                END DO
            END DO
        end do

        CALL write_qfile(mx,my,mz,meqn,xlow,ylow, zlow,&
                         dx,dy,dz,qc,iframe,ngrid,level,blockno,mpirank,dir3)
...
SUBROUTINE write_qfile(mx,my,mz,meqn,xlower,ylower, zlower,&
