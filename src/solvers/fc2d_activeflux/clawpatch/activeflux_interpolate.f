...
c     #
c     # Other routines :
c     # fclaw2d_clawpatch_compute_slopes 
c     # (for limited function reconstruction)
...
c     # cases.
c     # ----------------------------------------------------------
      subroutine activeflux_fort_interpolate_face
...
c                 # This is the right hand side of the LLSQ problem
     ...
     &                       pv(4)*shiftx(m)*shifty(m)
      ...
      subroutine activeflux_fort_interpolate_corner
...
c     # Conservative intepolation to fine grid patch
      subroutine activeflux_fort_interpolate2fine
...
c              # Fill in refined values on coarse grid cell (ic,jc)
     ...
     &                       pv(4)*shiftx*shifty
      ...
      if (manifold .ne. 0) then
         call activeflux_fort_fixcapaq2(mx,my,mbc,meqn,
...
c     # ------------------------------------------------------
      subroutine activeflux_fort_fixcapaq2(mx,my,mbc,meqn,
     &      qcoarse,qfine, areacoarse,areafine,igrid)
      implicit none
