...
c     ---------------------------------------------------------------
c>    @brief Determines the tranformation from a patch to it's
c>    same-sized neighbor's coordinate system
c>
c>    @param[in]  the pointer to the fclaw2d_patch_transform_data struct
c>    @param[out] a the 2x2 tranform matrix for the i,j indexes of a patch
c>    @param[out] f the transform vector
c     ---------------------------------------------------------------
      subroutine fclaw2d_clawpatch_build_transform_same(transform_ptr,
     &                                                  a,f)
      implicit none
