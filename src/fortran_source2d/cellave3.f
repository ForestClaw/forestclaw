      subroutine cellave3(blockno,xlow,ylow,zlow,dx,dy,dz,w)
      implicit none
...
c     # functions
      double precision cellave3_zeroin, fs

c     # For function fs
      common /comfs/ xl,xh,yl,yh,zl,zh

      integer blockno_com
      common /comcellave3/ blockno_com
...
c        # Fixed DAC : 12/20/2004
         icube(ipt) = int(sign(1.d0,fs(0.d0)))
...
c           # This is the only call to fs (and fdisc).
            s = cellave3_zeroin(0.d0,1.d0,fs,tol)
            edges(ipt1,ipt2) = s
...
c        # be no intersection at that edge.
         ...
         w = w + sgn*sign(1.d0,tri_normal(idir+1))
     &         *tri_normal(1)*dw
      enddo
...
c     # than criss-cross it.
      ...
      double precision xl,xh,yl,yh,zl,zh
      common /comfs/ xl,xh,yl,yh,zl,zh

      integer blockno
      common /comcellave3/ blockno
...
c     =================================================
      function cellave3_zeroin(ax,bx,f,tol)
c     =================================================
