...
\begin{figure}
\begin{center}
  \includegraphics[angle=-90,width=.7\columnwidth]{pdf/uni_advance_strong-crop}
  \\[1ex]
  \includegraphics[angle=-90,width=.7\columnwidth]{pdf/uni_exchange_strong-crop}
\end{center}
...
%  \\[1ex]
  \includegraphics[angle=-90,width=.7\columnwidth]{../figs_misc/advance_strong_omp}
\end{center}
\caption{Strong scaling of OpenMP parallelism for advection on a
  uniformly refined mesh:
  %Top: run time of advection Riemann problem kernel in both an optimized inlined interface and a generic function pointer interface via \manyclaw.  Bottom:
  MPI + OpenMp hybrid parallelization of the time integration.
}
\label{fig:ompscale}
\end{figure}%

In \Fig{ompscale}, we scale multi-node parallel calculations.
%and in future
%iterations on accelerators 
We use the \manyclaw set of patch-based PDE solvers.
%with
%\forestclaw.  
...
\Fig{spherical}, together with a strong scaling table.

\section{Conclusion}

We have presented a novel hybrid paradigm for AMR that achieves good
scaling both in the distributed and shared memory aspect of current
supercomputers.
%
% By
...
where the MPI version scales to 16,384 CPU cores.
...
% resolutions.
%
%the versatility of this approach the modular development of
%scalable codes.
...
\forestclaw was done.
The fourth author recognizes Simula National Lab for funding.
