...
\caption{The leaves in a forest of six octrees that serves as the computational
domain for the cubed sphere.  An adhoc refinement pattern has been 2:1 balanced
in terms of neighbor sizes and partitioned between five MPI processes (the
three middle ones are shown from black to white).}
\label{cubed3}
\end{figure}
...
implemented in \clawpack \cite{le:2002, clawpack}.  Optionally, the coordinates
of the patch can be geometrically mapped.  We assume a
...
Parallelization is not completed yet but envisioned for the final version of
the paper.
