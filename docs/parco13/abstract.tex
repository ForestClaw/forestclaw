...
\item The resulting parallel programming model is a hybrid and often referred
to as MPI+X.
\end{enumerate}
...
known to the mesh backend, from which it needs to be propagated to the
...
transparently by the backend.  Mesh modification directives, such as local
refinement and coarsening, are called from \forestclaw and relayed to the mesh
backend.
