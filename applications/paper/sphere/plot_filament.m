...
lstr = sprintf('t=0.0');
s = add_to_plot(fname,lstr,nf,'r','.',lsu,30,false);
nf = nf+1;
...
fname = 'filament.dat';
lstr = sprintf('TEST');
s = add_to_plot(fname,lstr,nf,'k','.','-',30,add2plot);
nf = nf+1;
...
s(nf) = add_to_plot(fname,lstr,nf,sym_color,'.',lsa,30,add2plot);
nf = nf+1;

add2plot = withlim(4) & levels(1) & adapt;
fname = 'level1_adapt_withlim_4_t2.5.dat';
lstr = sprintf('%.2f^o (A,4)',deg_eff);
s(nf) = add_to_plot(fname,lstr,nf,sym_color,'v',lsa,10,add2plot);
nf = nf+1;
...
function s = add_to_plot(fname,lstr,n,c,sty,lstyle,ms,add2plot)
