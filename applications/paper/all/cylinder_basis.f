...
c     #
      ...
      call map_comp2cylinder_derivs(x,y,theta,phi,
...
c             # d(t2)/dy = d(g*fy + gy*f)/dy         
      ...
      subroutine map_comp2cylinder(xc,yc,theta,phi)
      implicit none
...
c     # Map yc in [0,1] to phi in [-pi/2,pi/2]      
      ...
      call map_comp2cylinder_derivs(xc,yc,theta,phi,
     &               thetax, thetay, phix, phiy)

      end

      subroutine map_comp2cylinder_derivs(xc,yc,theta,phi,
     &      thetax, thetay, phix, phiy)
      implicit none
...
c     # Map yc in [0,1] to phi in [-pi/2,pi/2]      
      ...
      subroutine map_cylinder2comp(theta,phi,xc,yc)
      implicit none
...
c     # Map yc in [0,1] to phi in [-pi/2,pi/2]      
      ...
      subroutine map2cylinder(xp,yp,zp,theta,phi)
      implicit none
...
c      zp = r1*sin(pi2*yc)
      ...
      r1 = alpha*(1 + beta*sin(theta))
      if (r1 .eq. 0) then
          write(6,*) 'cylinder_basis.f : r1 .eq. 0'
          stop
      endif
...
c     # [0,2*pi]      
      ...
      call map2cylinder(xp,yp,zp,theta,phi)

      call map_cylinder2comp(theta,phi,xc,yc)
