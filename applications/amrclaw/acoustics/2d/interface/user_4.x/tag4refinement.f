      subroutine interface_tag4refinement(mx,my,mbc,meqn,
...
c     # Refine based only on first variable in system.
      ...
      do i = 1,mx
         do j = 1,my
            ...
            if (abs(q(i,j,mq)) .gt. 0.02d0) then
               tag_patch = 1
               return
            endif
         enddo
...
c     # We tag for coarsening if this coarsened patch isn't tagged for refinement
      subroutine interface_tag4coarsening(mx,my,mbc,meqn,
...
c            if (qmax - qmin .gt. 0.75d0) then
            if (abs(qcoarsened(i,j,mq)) .gt. 0.05d0) then
               tag_patch = 1
               return
            endif
         enddo
