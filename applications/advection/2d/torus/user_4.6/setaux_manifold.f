...
cc           # coordinates of lower left corner of grid cell:
c            xll = xlower + (i-1)*dx
c            yll = ylower + (j-1)*dy
c
cc           # difference stream function psi to get normal velocities:
c            aux(i,j,2) = -(psi(xll, yll+dy) - psi(xll,yll)) / dy
c            aux(i,j,3) =  (psi(xll+dx, yll) - psi(xll,yll)) / dx
         enddo
      enddo
