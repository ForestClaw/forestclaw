...
subroutine phasefield_fort_apply_bc(blockno, mx, my,mbc,mfields,xlower,ylower, &
...
!!   # This assumes that grad phi is zero at the boundary, so that we don't have a variable
!!   # coefficient that depends on phi.      
        ...
        do iface = 0,3
            if (intersects_bc(iface) .ne. 0) then
                ...
                elseif (bctype(iface) .eq. 2) then
                    ...
                    write(6,*) 'phasefield_apply_bc : bctype(iface) is not valid'
                    stop
                endif
                ...
                d = (a/2.d0 + b/h)  
                if (d .eq. 0) then
                    write(6,*) 'phasefield_fort_apply_bc : ill-defined BCs'
                    stop
                endif
                ...
                if (idir .eq. 0) then
                    ...
                    elseif (iface .eq. 1) then
                        ...
                        write(6,*) 'phasefield_apply_bc : ic, i1, ig are not initialized'
                        stop
                    endif
                ...
                elseif (idir .eq. 1) then
                    ...
                    elseif (iface .eq. 3) then
                        ...
                        write(6,*) 'phasefield_apply_bc : jc, j1, jg are not initialized'
                        stop
                    endif
                    !! location at interface
                    y = ylower + (j1 - 1)*dy
...
end subroutine phasefield_fort_apply_bc
