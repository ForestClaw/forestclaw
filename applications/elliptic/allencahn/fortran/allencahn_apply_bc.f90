...
subroutine allencahn_fort_apply_bc(blockno, mx, my,mbc,mfields,xlower,ylower, &
    ...
    do i = 1-mbc,mx+mbc
        do j = 1-mbc,my+mbc
            x = xlower + (i-0.5)*dx
            y = ylower + (j-0.5)*dy
            call allencahn_fort_beta(x,y,val_beta,grad_beta)
            beta(i,j,1) = val_beta
        end do
    end do
    ...
    do m = 1,mfields
        ...
        do iface = 0,3
            if (intersects_bc(iface) .ne. 0) then
                ...
                d = (a/2.d0 + b/h)  
                if (d .eq. 0) then
                    write(6,*) 'allencahn_fort_apply_bc : ill-defined BCs'
                    stop
                endif
...
end subroutine allencahn_fort_apply_bc
