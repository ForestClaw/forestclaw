...
c        # Split the flux difference into f-waves
         delta(1) = hunl-hunr

         sk = gamma/dy                  !# scaling factor
         delta(2) = hunl**2./hl + 0.5*g*hl**2.
     &         -hunr**2./hr - 0.5*g*hr**2.

c
         delta(3) = hunl*hutl/hl - hunr*hutr/hr

c        # modify df by source term for bathymetry:
         hsl = auxr(i-1,19)
         hsr = auxl(i,19)
         delta(2) = delta(2)+g*0.5*(hl+hr)*(hsr-hsl)
...
c
         ...
         s(i,1) = (u(i)-a(i))*sk
         fwave(i,1,1) = a1*sk
         fwave(i,2,1) = (a1*(u(i)-a(i))*enx + a1*v(i)*etx)*sk
         fwave(i,3,1) = (a1*(u(i)-a(i))*eny + a1*v(i)*ety)*sk
         fwave(i,4,1) = (a1*(u(i)-a(i))*enz + a1*v(i)*etz)*sk

c
         s(i,2) = u(i) * sk
         fwave(i,1,2) = 0.0d0
         fwave(i,2,2) = a2*etx*sk
         fwave(i,3,2) = a2*ety*sk
         fwave(i,4,2) = a2*etz*sk

c
         s(i,3) = (u(i)+a(i)) * sk
         fwave(i,1,3) = a3*sk
         fwave(i,2,3) = (a3*(u(i)+a(i))*enx + a3*v(i)*etx)*sk
...
c        # Is this really necessary here?
         do i=2-mbc,mx+mbc
            erx = auxr(i-1,14)
            ery = auxr(i-1,15)
            erz = auxr(i-1,16)
