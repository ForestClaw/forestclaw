...
SUBROUTINE clawpack46_rpt2_manifold(ixy,maxm,meqn,mwaves,mbc,mx, &
    ...
    do i = ix1,ixm1
        ...
        h(i) = ql(i1,1)
        u(i) = (enx(i)*ql(i1,2)+eny(i)*ql(i1,3)+enz(i)*ql(i1,4)) / h(i)
        v(i) = (etx(i)*ql(i1,2)+ety(i)*ql(i1,3)+etz(i)*ql(i1,4)) / h(i)
        a(i) = dsqrt(grav*h(i))
    enddo
    ...
    do m = 1,meqn
        do i = ix1,ixm1
            bpasdq(i,m) = 0.d0
            do mw = 1,mwaves
                bpasdq(i,m) = bpasdq(i,m) + max1(s(i,mw),0.d0)*wave(i,m,mw)
            END DO
        END DO
    END DO
    ...
    DO m = 1,meqn
        do i = ix1,ixm1
            bmasdq(i,m) = 0.d0
            do mw = 1,mwaves
                bmasdq(i,m) = bmasdq(i,m) + dmin1(s(i,mw), 0.d0)*wave(i,m,mw)
            END DO
        END DO
    END DO
