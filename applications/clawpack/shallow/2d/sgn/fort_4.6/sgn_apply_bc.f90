...
subroutine sgn_fort_apply_bc(blockno, mx, my,mbc,mfields,xlower,ylower, &
...
!!   # Here, we set dD/dn = 0 at the boundary.  For the SGN operator, this isn't
!!   # exactly the same as setting the flux to zero at the boundary.
!!   # TODO : Fix  this
    ...
    do m = 1,mfields
        ...
        do iface = 0,3
            if (intersects_bc(iface) .ne. 0) then
                ...
                d = (a/2.d0 + b/h)  
                if (d .eq. 0) then
                    write(6,*) 'sgn_fort_apply_bc : ill-defined BCs'
                    stop
                endif
        ...
        !! Ghost cells now all filled in.  Now apply Laplace operator.   
        !! Here, we are cheating and not applying the SGN operator.  But 
        !! at physical boundaries, this shouldn't matter too much ...

        !! ccheck : Conservation check.  
        !! This will sum up flux around physical boundary.  This sum
        !! should equal sum of RHS for conservation.
...
end subroutine sgn_fort_apply_bc
