...
void def_patch(int ncid, int subgroupid, int patchidx,
...
void write_q(int rank, int groupid, int mx)
{
    int retval;
    double q[mx];
    /* Create pretend data*/
...
int main(int argc, char *argv[])
{
    ...
    if (mpi_rank == 0)
       printf("*** tst_parallel testing very basic parallel access.\n");

    /* Create a parallel netcdf-4 file. */
    if ((retval = nc_create_par(filename, NC_NETCDF4|NC_MPIIO, MPI_COMM_WORLD, 
                                MPI_INFO_NULL, &ncid)))
        ERR(retval);
    ...
    for (int i = 0; i < numpatch; ++i)
    {
        ...
        if ((retval = nc_def_grp(ncid, patchname, &subgroupid[i])))
            ERR(retval);
        /* According to the website, all netCDF metadata writing operations 
          are collective. That is, all creation of groups, types, variables, 
          dimensions, or attributes. */
        def_patch(ncid, subgroupid[i], i, time, numdim, meqn, maux, mx);
    }

    groupid_for_rank = subgroupid[mpi_rank];
    write_q(mpi_rank, groupid_for_rank, mx);
    
    // /* Close the file. */
    if ((retval = nc_close(ncid)))
        ERR(retval);
    if (mpi_rank == 0)
        printf("*** SUCCESS writing file %s in parallel!\n", filename);
